todo: в учебнике это на стр.88-89

# Генерация значений на стороне БД

При сохранении объектов некоторые поля может генерировать БД. Например, время создания \ последнего изменения объекта. Id сюда не относится, это отдельная тема. Чтобы сказать хиберу вытянуть эти "автозначения" сразу после сохранения объекта, используем аннотацию `@Generated` в паре с настройками `@Column`:
