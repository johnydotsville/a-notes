...
* :red_circle: - чтобы понять раздел, нужно вернуться в него после прохождения другого раздела.
...
- [ ] Стр.145: Пример на мапинг Set. Показано как сделать мапинг на одно поле таблицы. В сущности Товар есть коллекция `Set<String>`, в которую выбирается из таблицы картинок только название картинки. Ценность не понятна, потому что фишка с зависимым жизненным циклом очевидно здесь не будет работать.
- [ ] Стр.146: Хибер-фича, показан "bag" - список без гарантии порядка элементов.
- [ ] Стр.147: Мапинг List'а на том же примере Товар - Картинка.
