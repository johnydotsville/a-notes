...
- [ ] Стр.186: Связь через третью таблицу, но без третьего класса.
...
- [x] Стр.230 описаны основные термины: persistence context, unit of work, persistence life cycle.
- [x] Стр.231 приведена диаграмма перехода состояний сущности и вкратце рассказано о всех состояниях.
- [ ] Стр.232 написано что контекст работает как кэш первого уровня. Не очень понятно написано как работает синхронизация с БД, поэтому оставляю пункт анчекнутым.

### 10.2 Entity manager interface

- [ ] Стр.234 показан каноничный способ организовать unit of work с транзакцией. Показано правда на примере с JTA, что довольно сложно. Поэтому оставляю анчекед, т.к. пользуюсь обычной транзакцией из em.
- [x] Стр.236 показано сохранение сущности, рассказано как из БД добывается id при вызове em.persist() при разных стратегиях генерации.
- [ ] Стр.236: Рассказано как узнать состояние сущности: хранимое, отсоединенное, транзиентное. Правда написано криовато, лучше загуглить отдельно будет.
- [x] Стр.237: Рассказано про dirty-check, про снапшот сущности.
- [ ] Стр.239: .getReference(), получение ссылки на сущность без обращения к БД. Написано мало и не понятно. Сказано, что подробнее будет в главе 12. Пусть здесь упоминание висит, мб сюда можно будет вернуться и что-нибудь дочерпнуть.
- [ ] Стр.240: про удаление сущностей. Есть мутная настройка `hibernate.use_identifier_rollback`, дает ошибку. В конспекте я упомянул что нужно вернуться к ней после раздела про работу с detached-состоянием. :red_circle:
- [x] Стр.241: рефреш сущности из БД.
- [ ] Стр.242: репликация данных. Хибер-фича, позволяющая извлечь сущность из одного контекста и сохранить в другой. В другой - в смысле совсем в другой, связанный с другой БД.
- [ ] Стр.243: кэширование в контексте. Рассказано, что при бездумной загрузке сущностей хибер может потреблять много памяти. Даны советы как этого избежать. Методы detach и clear.

### 10.3 Working with detached state

- [x] Стр.245-250. Особенности реализации equals и hashCode для сущностей. Поместил это в конспект про маппинги, т.к. основной материал по формированию сущностей именно там.
