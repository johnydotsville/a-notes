...
Вместо лямбды указываем статический метод класса, чтобы параметры этого метода подходили под параметры метода ФИ.
...
Вместо лямбды указываем обычный метод класса, чтобы параметры этого метода подходили под параметры метода ФИ. Метод указываем, используя существующий экземпляр класса.
...
> Долго не мог придумать, как назвать этот случай, потому что по сути это тоже вызов метода экземпляра, но только хитрее, чем в прошлом пункте. Названия, найденные в интернете, какие-то запутанные. В итоге я придумал название "метод НА экземпляре", которое имхо хорошо показывает, в чем тут прикол.

Оригинальное название: Reference to an instance method of an arbitrary object of a particular type. "Ссылка на обычный метод *какого-то* объекта определенного типа".

Вместо лямбды мы указываем *обычный* метод класса, но используем для этого *тип*. Хитрость этого подхода в том, что оно не совсем очевидно преобразуется в вызов указанного метода: указанный метод вызывается на *первом аргументе ФИ*, а остальные аргументы ФИ служат параметрами указанному методу.
