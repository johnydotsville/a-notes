...
Это методы объекта функции. Оба они позволяют вызвать функцию, явно указав объект, который надо положить в this. Отличие в том, что call принимает параметры для функции отдельными значениями, а apply - массивом (и псевдомассивом):
...
```javascript
apply(context, [arg1, arg2, argN]);  // Параметры - массивом и псевдомассивом
```

> Как запомнить, где массив, а где отдельные значения? В apply первые буквы `app` похожи на `arr`, значит в apply используется псевдомассив. Ну а в call, соответственно, отдельные значения.

apply берет элементы массива и позиционно передает их в параметры функции.

Сегодня, когда есть spread оператор, метод apply можно полностью заменить на call вот так:

```javascript
call(context, ...params)
```

apply остался с тех времен, когда spread-оператора не было и надо было как-то передавать массив. Т.е. кроме способа передать аргументы, различий у call и apply нет.
