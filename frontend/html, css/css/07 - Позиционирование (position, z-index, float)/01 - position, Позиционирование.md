...
* Для наблюдения sticky-эффекта нужно, чтобы появился скроллинг.
* Тогда когда между стики-элементом и границей скролящегося родителя оказывается указанное расстояние, элемент "залипает" на месте и выглядит с виду как fixed.
  * Расстояние задается стики-элементу через top, left, right или bottom.
  * Это расстояние - от скролящегося родителя.
* Когда контейнер прокручивается до конца и уходит из области видимости, стики-элемент уходит вместе с ним.
