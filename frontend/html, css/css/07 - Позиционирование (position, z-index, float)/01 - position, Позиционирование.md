...
* Позиционирующие свойства задают расстояние от краев окна, при котором элемент должен залипнуть на месте.
  * Положение можно указать от любого края, не только от верха. Верх просто выбран для примера как самый наглядный.
* Когда в результате прокрутки расстояние становится как заданное, элемент залипает на экране на одном месте.
* Когда прокрутка достигает конца родителя и уходит дальше, то липкий элемент остается в границах родителя и при дальнейшей прокрутке страницы его уже не видно.
...
* sticky-элемент не ограничивает видимую область родителя, поэтому при прокрутке другие элементы оказываются непосредственно под sticky-элементом и происходит визуальное наложение одного на другое. Чтобы это скрыть, можно ставить фоновый цвет sticky-элементу.
